 ...
 * 作者: zhangtao,200803
 * 修改：
 *  20080504 移动时，修改被移节点的parent_tree_id
 ************************************************************************/
create procedure dbo.sp_tree
(
	@table nvarchar(30), 	/* 表名 */
	@operation_type int,	/* 操作类型 1: 查询; 2: 添加; 3:删除; 4: 更新; 5: 移动; 6: 拷贝 */
...
--操作：查询子节点
--参数: 	@table , @tree_id
--返回： 数据集(包含is_has_children表示该节点是否含有子结点）
if(@operation_type = 1)
begin
	select @sql = 'select distinct t1.tree_id,t1.parent_tree_id,t1.name,t1.level_code,case when t2.tree_id is null then 0 else 1 end as is_has_children from ' 
		+ @table 
		+ ' as t1 left join '
		+ @table 
		+ ' as t2 on t1.tree_id = t2.parent_tree_id where t1.parent_tree_id = ' 
		+ ltrim(str(@tree_id))
		+ ' group by t1.tree_id,t1.parent_tree_id,t1.name,t1.level_code,t2.tree_id'
	print @sql
...
begin
	...
	begin
		select @level_code = '' 
		select @parent_tree_id = -1
	end
	else
	begin
		select @sql = 'select @parent_tree_id = tree_id from ' + @table + ' where level_code = ''' + @level_code + ''''
    exec dbo.sp_executesql @sql,N'@parent_tree_id int out', @parent_tree_id out	
	end
  
  --修改被移节点的父ID
  select @sql = 'update ' + @table + ' set parent_tree_id = ' + ltrim(str(@parent_tree_id)) 
		+ ' where tree_id = ' + ltrim(str(@tree_id))
	exec dbo.sp_executesql @sql
	
	--修改被移节点及其所有子孙节点的层次码
	select @level_code = @level_code + replace(str(@tree_id, 5), space(1), '0')
	select @sql = 'update ' + @table + 
		' set level_code=replace(level_code,''' + @old_leve_code + ''', ''' + @level_code + ''') '
		+ ' where level_code like ''' + @old_leve_code + '%'''
	exec dbo.sp_executesql @sql
	return 
end
