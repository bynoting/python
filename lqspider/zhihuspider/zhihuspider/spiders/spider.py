...
class zhihusipder(CrawlSpider) :
    ...
    # Todo: summary allow 中的正则比较宽泛，默认已经有包含的感觉。。
    rules = (
        Rule(SgmlLinkExtractor(allow = ('/question/\d+#.*?', )), callback = 'parse_page', follow = True),
        Rule(SgmlLinkExtractor(allow = ('/question/\d+', )), callback = 'parse_page', follow = True),
        # Rule(SgmlLinkExtractor(allow=('/zhihu.com', )), callback = 'parse_page', follow = True),
    )
    headers = {
    "Accept": "*/*",
    ...
    def parse_page(self, response):
        ...
        # print item['name']
        # item['title'] = selecter.xpath('//h2[@class="zm-item-title zm-editable-content"]/text()').extract()
        ...
        item['name'] = selecter.xpath('//h3/a[2]/text()').extract()
